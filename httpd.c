...
static int node_c_on_message_begin(http_parser *parser)
{
	...
	DBGPRINT(EDEBUG,("[Trace@HTTPD] dwUsrID= %d Http-Method=%d\r\n",((usr_t*)node->usr)->dwUsrID,parser->method));
...
static int _channel_callback(void *pUser, void *msg, unsigned int size)
{
    ...
    case _EVDATA:
		DBGPRINT(EDEBUG,("[Trace@HTTPD] __EVDATA-SIZE=%d\r\n",dataqueue_datasize(msgChannel->u.dataqueue)));
        while(0<dataqueue_datasize(msgChannel->u.dataqueue)){
            ...
            if(0==_node_parser(node, buffer, len, &pos)){					
				if(node->status!=estatnodeeady){
					DBGPRINT(EERROR,("[Trace@HTTPD] Error dwUsrID=%d\r\n",usr->dwUsrID));
	...
	case _EVSENT:
		if(NULL!=usr->node){
			...
			if(node->sendsize!=node->sentsize){
				DBGPRINT(EMSG,("[Trace@HTTPD] Waiting SEnd [sendsize=%u,sentsize=%d], dwUsrID=%u\r\n", node->sendsize,node->sentsize,usr->dwUsrID));
				break;
			}
			if(node->status==estatushandle){				
				DBGPRINT(EDEBUG,("[Trace@HTTPD] Continue2 Node! dwUsrID=%u\r\n",usr->dwUsrID));
				httpD->handler((node_t*)node,ENODECONTINUE,msgChannel->u.size,0);
			}else if(node->status==estatusdone){				
				DBGPRINT(EMSG,("[Trace@HTTPD] Finish Node! dwUsrID=%u\r\n",usr->dwUsrID));
				usr->node = NULL;
				httpD->handler((node_t*)node,ENODECLEAR,0,0);
				node_free(node);
			}else{
				DBGPRINT(EERROR,("[Trace@HTTPD] _EVSENT ERROR!! (badSTatus=%d) dwUsrID=%u\r\n",node->status,usr->dwUsrID));
			}
		}else{
			DBGPRINT(EERROR,("[Trace@HTTPD] _EVSENT ERROR!! NO NODE FOUND, dwUsrID=%u\r\n",usr->dwUsrID));
		}
		break;
    case _EVCLOSED:
		list_del(usr); //Remove
